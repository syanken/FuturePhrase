...
| LangChain-OpenAI | 0.2.0 | OpenAI 模型集成 |
...
### 1. 克隆仓库
```bash
git clone https://github.com/yourusername/FuturePhrase.git
cd FuturePhrase
```
...
其他运行时配置可见于 `server/README` 或 `server/app/main.py` 中的 `dotenv` 加载逻辑。
